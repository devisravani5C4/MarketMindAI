document.addEventListener("DOMContentLoaded", function () {
    ...
    // ==========================================
    // 7. Review Sentiment Form Handling (AJAX)
    // ==========================================
    ...
    if (revForm) {
        revForm.addEventListener("submit", function (e) {
            ...
            fetch("/run-reviews", {
                ...
                .then(res => {
                    if (res.status === "success") {
                        const posEl = document.getElementById("pos-pct");
                        const neuEl = document.getElementById("neu-pct");
                        const negEl = document.getElementById("neg-pct");

                        if (posEl) posEl.textContent = res.data.global.positive + "%";
                        if (neuEl) neuEl.textContent = res.data.global.neutral + "%";
                        if (negEl) negEl.textContent = res.data.global.negative + "%";

                        const tbody = document.getElementById("product-sentiment-tbody");
                        if (tbody && res.data.product_breakdown) {
                            tbody.innerHTML = res.data.product_breakdown.map(row =>
                                `<tr><td>${Object.values(row)[0]}</td><td>${row.Total}</td><td class="text-success">${row['Positive %']}%</td><td class="text-danger">${row['Negative %']}%</td></tr>`
                            ).join("");
                        }

                        document.getElementById("sentiment-results")?.classList.remove("d-none");
                    } else {
                        alert("Sentiment analysis error: " + (res.error || "Failed to run review analysis."));
                    }
                })
                .catch(err => console.error("Error:", err));
        });
